				...
				<div class="flex items-center justify-between py-4 lg:gap-6 md:flex-row md:gap-x-4" aria-label="Верхнее меню">
					...
					<div class="flex items-center gap-5 lg:w-full lg:justify-between xs:w-auto">
						<div class="flex items-center gap-x-2.5 sm:hidden">
							<Combobox as="div" v-model="selectedCity" @update:modelValue="query = ''" class="flex items-center gap-2">
								...
								<div class="relative 2xl:w-52">
									<ComboboxInput
										...
										@change="query = $event.target.value" @blur="query = ''" :display-value="(city) => city?.name" />
									<ComboboxButton
									...
									<ComboboxOptions v-if="filteredCityes.length > 0"
										...
										<ComboboxOption v-for="city in filteredCityes" :key="city.id" :value="city" as="template"
				...
				<div class="relative flex items-center justify-between py-4">
					...
					<nav class="flex items-center justify-between  gap-x-12" aria-label="Основное меню">
						...
						<PopoverButton type="button"
							...
							Каталог
							<Icon name="material-symbols:keyboard-arrow-down-rounded" class="w-6 h-6  group-hover:text-red-600 "
								aria-hidden="true" />
						</PopoverButton>
