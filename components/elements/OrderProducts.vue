...
<template>
    <div class="max-h-80 overflow-y-auto border border-gray-200 rounded-lg mt-10">
        <table class="min-w-full divide-y divide-gray-300 border-collapse pr-1">
            ...
            <tr v-for="item in products" :key="item.id" v-if="products.length" class="hover:bg-amber-100 transition cursor-pointer" @click="navigateTo('/catalog/' + item.product.slug_path); popupStore.close('modal')">
