                        ...
                        <Combobox @update:modelValue="onSelect">
                            <div class="relative">
                                ...
                                <ComboboxInput
                                    ...
                                    @keydown.enter="goToPageSearch(searchInput)"
                                    @blur="query = ''"/>
                                <NuxtLink class="absolute right-4 top-3.5" :to="`/search?find=${searchInput}`"
                                          @click="popupStore.close('search')">
                                    <Icon name="mdi-light:tab-plus" class="pointer-events-none text-gray-600 h-5 w-5"
                                          aria-hidden="true">
                                    </Icon>
                                </NuxtLink>
                            </div>
...
function goToPageSearch(value) {
    if (!value) return

    navigateTo(`/search?find=${value}`)
